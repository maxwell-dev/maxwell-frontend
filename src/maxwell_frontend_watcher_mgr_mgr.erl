...
%%%-------------------------------------------------------------------
-module(maxwell_frontend_watcher_mgr_mgr).
...
fetch_watcher_mgr(Type) ->
  gen_server:call(?SERVER, {fetch_watcher_mgr, Type}).
...
handle_call({fetch_watcher_mgr, Type}, _From, State) ->
  reply(fetch_watcher_mgr2(Type, State));
...
handle_info({'EXIT', FromPid, Reason}, State) ->
  lager:info("Watcher mgr was down: reason: ~p", [Reason]),
  noreply(on_watcher_mgr_down(FromPid, State));
...
fetch_watcher_mgr2(Type, State) ->
  case dict:find(Type, State#state.type_pids) of
    error ->
      case maxwell_frontend_watcher_mgr:start_link(Type) of
        {ok, Pid} = Result ->
